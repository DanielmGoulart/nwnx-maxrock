...
void * (__cdecl *nwnx_malloc)( unsigned int size ) = (void *(__cdecl *)(unsigned int))0x0040D550;
//Frees memory blocks allocated by nwnx_malloc/calloc/realloc
void (__cdecl *nwnx_free)( void * ptr  ) = (void (__cdecl *)(void *))0x0040D560;
...
int CNWNXAreas::LoadArea(char* ResRef) {
	...
	CNWSModule *pModule = (CNWSModule*)(pMod);
	...
	CNWSArea *pArea = (CNWSArea*)nwnx_malloc(0x20C);
	...
	CExoArrayList_uint32___Add(&pModule->mod_areas, NULL, nAreaID);
...
int CNWNXAreas::DestroyArea(nwn_objid_t nAreaID) {
	...
	while (!Objects.empty()) {
		...
		Objects.pop_back();
		pObject = (CGameObject*)((*NWN_AppManager)->app_server->srv_internal)->GetGameObject(nTmpObj);
		if (!pObject) continue;
		...
		if (!Area) {
			void (__fastcall *pDestructor)(CGameObject *pTHIS, void *pVOID, int flag) = (void (__fastcall*)(CGameObject *pTHIS, void *pVOID, int flag))(*(DWORD*)((DWORD*)(pObject->obj_type)));
	...
	//void *pArray = ((uint32_t *)pModule+0x6);
	CExoArrayList_uint32___Remove(&pModule->mod_areas, NULL, nAreaID);
...
CNWSArea *CNWNXAreas::GetOverrideArea(CNWSCreature *Cre) {
	...
	_log(3, "Cre: %08X\n", Cre);
	if (Cre) {
		CNWSScriptVarTable *svTable = &Cre->obj_vartable;
		CScriptLocation Loc;
		Loc.loc_area = OBJECT_INVALID;
	...
	if ((pModule->mod_PlayerTURDList.ListHeader) && (pModule->mod_PlayerTURDList.ListHeader->Count > 0)) {
		...
		while (Element) {
			...
			if (TURD->AreaData.MiniMap) {
				TURD->AreaData.RemoveArea(nAreaID);
			}
